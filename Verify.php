...
class Verify extends Client
{
    ...
    */
    ...
    public function getUrlControl()   //new stuff
    {
        $this->uri = '/verify/control/';
        return $this->api . $this->uri . $this->format;
    }
    ...
    public function sendVerifyControl($request_id, $cmd)  // new stuff
    {
        $params = ['request_id' => $request_id, 'cmd' => $cmd]; //required parameter
        $this->response = $this->request($this->getUrlControl(), $this->getEncodedParams($params));
        return $this->response;
    }
